                                            ...
                                            int* exportedRows)
{
    ...
    QList<BusinessExportRow> exportRows = rows;
    BusinessWorkbookResolver::prepareRowsForExport(exportRows);

    if (!BusinessCsvWriter::writeRowsToCsv(exportRows, filePath, errorMessage)) {
        errorMessage = BusinessWorkbookResolver::formatSheetError(sheetName, errorMessage);
        return false;
    }

    if (exportedRows != nullptr) {
        *exportedRows = exportRows.size();
    }
